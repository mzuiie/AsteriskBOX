...
abstract class UpdateResourceFileAssetsTask : DefaultTask() {
    ...
    /** 本地预置内核目录（<dir>/<abi>/libsing-box.so），存在则优先于 GitHub 下载。 */
    @get:Optional
    @get:InputFiles
    abstract val localSingBoxCoreDir: DirectoryProperty
    ...
    fun updateAssets() {
        AndroidSingBoxAssets.forEach { asset ->
            ...
            if (copyLocalSingBoxCore(asset, target)) {
                logger.lifecycle("sing-box core (local prebuilt): ${asset.androidAbi}")
            } else {
    ...
    private fun copyLocalSingBoxCore(asset: SingBoxAsset, target: File): Boolean {
        val local = localSingBoxCoreDir.orNull
            ?.file("${asset.androidAbi}/libsing-box.so")
            ?.asFile
            ?: return false
