...
internal object VpnSingBoxConfigFactory {
    fun create(
        ...
        return VpnServiceStartConfig(
            ...
            // v6 地址常驻(与 tun address 同源): 关 IPv6 时也要建 v6 路由收流量进隧道防旁路
            ...
            singBoxTunStack = SingBoxConfigFactory.tunStack(appState),
...
internal fun buildVpnHevSocks5TunnelConfig(
    ...
    return HevSocks5TunnelConfig(
        ...
        mtu = tunOptions.mtu,
        ipv4Address = tunOptions.ipv4Address.address,
        // v6 常驻收口: 与主配置同, v6 进隧道后由路由规则拒, 防物理旁路泄露
        ipv6Address = tunOptions.ipv6Address.address,
