...
class AsteriskApplication : Application(), SingletonImageLoader.Factory {
    ...
    private companion object {
        const val CrashLogTag = "Crash"
    }

    /**
     * 崩溃黑匣子：任何线程的未捕获异常（含协程漏网异常）先把完整堆栈写进应用日志
     * （AndroidLogcatRepository 落盘文件，重启后应用日志页仍可见），再交回系统流程。
     * 没有它，闪退现场只存在于 logcat，用户永远拿不到死因。
     */
    private fun installCrashBlackBox() {
        val frameworkHandler = Thread.getDefaultUncaughtExceptionHandler()
        Thread.setDefaultUncaughtExceptionHandler { thread, error ->
            runCatching {
                AndroidAppLogger.error(
                    CrashLogTag,
                    "未捕获异常（thread=" + thread.name + "）type=" + error.javaClass.name,
                    error,
                )
            }
            frameworkHandler?.uncaughtException(thread, error)
        }
    }
