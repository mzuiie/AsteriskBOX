        ...
        return try {
            val source = if (sourceContent == null) {
                ...
                withContext(Dispatchers.IO) {
                    val viaProxy = fetchOptions.toHttpProxy()
                    // N10: 经隧道代理拉取失败时回落直连重试一次(保可用性, 跳点分析 J1)
                    runCatching {
                        ...
                        if (viaProxy == null) throw error
                        AndroidAppLogger.warn(
                            tag = "Subscription",
                            message = "Proxy fetch failed, falling back to direct: ${error.message}",
                        )
                        downloadSubscription(
                            sourceUrl = sourceUrl,
                            userAgent = userAgent,
