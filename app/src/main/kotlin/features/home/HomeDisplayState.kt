...
internal data class HomeControllerState(
    ...
    val runMode: Int,
    val singBoxMode: Int,
    val freeFlowActive: Boolean = false,
)
...
internal fun buildHomeControllerState(
    ...
    freeFlowActive = appState.isFreeFlowChannelApplied(),
)
...
internal fun buildHomeModeChange(
    ...
    // T1c: 免流存续时直连模式=全量计费跳点，模型层拒切（芯片置灰是第一道闸）
    if (requestedMode == SingBoxModeDirect && appState.isFreeFlowChannelApplied()) return null
