...
internal abstract class AppStateDao {
    ...
    /** 免流声明档单行读写（features.freeflow 专用，与 AppState 提交链解耦）。 */
    open suspend fun loadFreeFlowProfile(): FreeFlowProfileEntity? = findFreeFlowProfile()

    open suspend fun saveFreeFlowProfile(entity: FreeFlowProfileEntity) {
        upsertFreeFlowProfile(entity)
    }

    open suspend fun clearFreeFlowProfile() {
        deleteFreeFlowProfile()
    }

    @Query("SELECT * FROM free_flow_profile WHERE id = 1")
    protected abstract suspend fun findFreeFlowProfile(): FreeFlowProfileEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    protected abstract suspend fun upsertFreeFlowProfile(entity: FreeFlowProfileEntity)

    @Query("DELETE FROM free_flow_profile")
    protected abstract suspend fun deleteFreeFlowProfile()
